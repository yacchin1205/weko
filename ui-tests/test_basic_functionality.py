...
class TestBasicFunctionality:
    """Basic functionality tests for WEKO3."""

    def test_homepage_loads(self, page: Page, base_url: str):
        """Test that the homepage loads successfully."""
        page.goto(base_url)
        expect(page).to_have_title(lambda title: "WEKO" in title or len(title) > 0)
    ...
    def test_search_functionality(self, page: Page, base_url: str):
        ...
        if search_input and search_input.is_visible():
            ...
            expect(page).to_have_url(lambda url: "search" in url.lower() or "q=" in url)
